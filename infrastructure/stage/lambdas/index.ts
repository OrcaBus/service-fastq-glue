...
export function buildLambdaFunction(scope: Construct, props: BuildLambdaProps): LambdaObject {
  ...
  if (props.lambdaName == 'deleteBasespaceRunData') {
    ...
    NagSuppressions.addResourceSuppressions(
      ...
      [
        {
          id: 'AwsSolutions-IAM5',
          reason: 'This lambda requires read access to BaseSpace SSM parameter and secret.',
        },
      ],
      true
    );
  ...
  if (props.lambdaName == 'getArchiveDestinationUri' && props.archiveBucketName) {
    lambdaFunction.addEnvironment('ARCHIVE_BUCKET', props.archiveBucketName);
  }
